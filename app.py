...
.badge-high   { background:#c53030; color:white; padding:3px 10px; border-radius:20px; font-size:0.8rem; font-weight:600; }
.badge-medium { background:#c47800; color:white; padding:3px 10px; border-radius:20px; font-size:0.8rem; font-weight:600; }
.badge-low    { background:#1D9E75; color:white; padding:3px 10px; border-radius:20px; font-size:0.8rem; font-weight:600; }
div[data-testid="stSidebar"] { background: linear-gradient(180deg, #0f2027, #203a43, #2c5364); }
div[data-testid="stSidebar"] * { color: white !important; }
</style>
""", unsafe_allow_html=True)

DISEASE_INFO = {
    'MEL':   {'name':'Melanoma','severity':'HIGH','color':'#f56565','referral':True,'icon':'⚠️',
              ...
              'treatment':['🏥 Immediate dermatologist consultation — do not delay','🔪 Surgical excision with wide safety margins','🧬 Sentinel lymph node biopsy','💉 Immunotherapy or targeted therapy if advanced','☀️ Strict sun avoidance and SPF 50+ daily','📅 Full-body skin checks every 3 months'],
              'risk_factors':['UV radiation exposure','Family history','Fair skin','Multiple moles']},
    'NV':    {'name':'Melanocytic Nevi (Mole)','severity':'LOW','color':'#48bb78','referral':False,'icon':'✅',
              ...
              'treatment':['✅ Generally benign — no immediate treatment needed','👁️ Monitor using ABCDE rule monthly','☀️ Apply broad-spectrum SPF 30+ daily','📅 Annual dermatologist skin check recommended','🚫 Avoid picking or irritating the mole'],
              'risk_factors':['Sun exposure','Genetics','Hormonal changes']},
    'BCC':   {'name':'Basal Cell Carcinoma','severity':'MEDIUM','color':'#f6ad55','referral':True,'icon':'⚡',
              ...
              'treatment':['🏥 Consult dermatologist within 2-4 weeks','🔬 Mohs micrographic surgery — gold standard','📡 Radiation therapy for inoperable cases','💊 Topical treatments for superficial BCC','☀️ SPF 50+ and protective clothing'],
              'risk_factors':['Cumulative UV exposure','Fair skin','Age >50','Immunosuppression']},
    'AKIEC': {'name':'Actinic Keratoses','severity':'MEDIUM','color':'#f6ad55','referral':True,'icon':'⚡',
              ...
              'treatment':['🏥 Dermatologist consultation recommended','❄️ Cryotherapy — most common treatment','💊 Topical: 5-fluorouracil or imiquimod','💡 Photodynamic therapy for widespread lesions','☀️ Daily SPF 50+ essential'],
              'risk_factors':['Chronic sun exposure','Fair skin','Outdoor occupation','Age >40']},
    'BKL':   {'name':'Benign Keratosis','severity':'LOW','color':'#48bb78','referral':False,'icon':'✅',
              ...
              'treatment':['✅ Harmless — no medical treatment necessary','✂️ Removal for cosmetic reasons via cryotherapy','🧴 Keep skin well-moisturized','👁️ Monitor for rapid changes','📅 Routine annual skin check'],
              'risk_factors':['Aging','Sun exposure','Genetics']},
    'DF':    {'name':'Dermatofibroma','severity':'LOW','color':'#48bb78','referral':False,'icon':'✅',
              'description':'Common benign fibrous nodules, often appearing on the legs. Usually painless and completely harmless.',
              'treatment':['✅ Benign — no treatment required','🔪 Surgical removal if causing discomfort','💉 Corticosteroid injections to flatten lesion','🚫 Avoid repeated irritation','👁️ Monitor for unexpected growth'],
              'risk_factors':['Minor skin injuries','Insect bites','Female sex']},
    'VASC':  {'name':'Vascular Lesion','severity':'LOW','color':'#63b3ed','referral':False,'icon':'ℹ️',
              'description':'Blood vessel abnormalities in the skin. Most are harmless birthmarks or acquired lesions.',
              'treatment':['✅ Most vascular lesions are benign','💡 Pulsed dye laser — most effective','💉 Sclerotherapy for larger lesions','🔪 Surgical removal for isolated cases','🏥 Consult if growing rapidly or bleeding'],
              'risk_factors':['Genetics','Aging','Hormonal changes','Sun damage']},
}

IMG_SIZE  = 224
ONNX_ID   = '11aSFFVn4SnazgRwehKL7hRzOujxuBdvA'
ONNX_PATH = 'skin_model.onnx'

# ─── Download ONNX model ───────────────────────────────────────
if not os.path.exists(ONNX_PATH):
    with st.spinner('🔄 Loading AI model for first time... (~1 min)'):
        gdown.download(f'https://drive.google.com/uc?id={ONNX_ID}',
                       ONNX_PATH, quiet=False)
...
def predict(session, le, image):
    img   = np.array(image.convert('RGB'))
    img   = cv2.resize(img, (IMG_SIZE, IMG_SIZE))
    img   = img / 255.0
    img   = np.expand_dims(img, 0).astype(np.float32)
    inp   = session.get_inputs()[0].name
    preds = session.run(None, {inp: img})[0][0]
    top3  = np.argsort(preds)[::-1][:3]
    results = []
    for idx in top3:
        code = le.classes_[idx]
        info = DISEASE_INFO.get(code, {})
        results.append({'code':code,'name':info.get('name',code),
...
with st.sidebar:
    ...
    page = st.radio("Navigation", [
        "🏠  Diagnose","📅  Monitor","📊  Analytics","ℹ️  About"
    ...
    st.divider()
    st.markdown("<div style='font-size:0.75rem;color:#718096;text-align:center'>⚕️ Educational use only.<br>Always consult a dermatologist.</div>", unsafe_allow_html=True)
...
if "Diagnose" in page:
    ...
    with col1:
        ...
        uploaded = st.file_uploader("Upload skin image", type=['jpg','jpeg','png'], label_visibility='collapsed')
        if uploaded:
            image = Image.open(uploaded)
            st.image(image, use_container_width=True, caption="Uploaded — ready for analysis")
            st.markdown("<br>", unsafe_allow_html=True)
            add_mon   = st.checkbox("📅 Add to weekly monitoring tracker")
            spot_name = st.text_input("Spot name", "Spot 1") if add_mon else ""
            ...
            if run:
                if model is None:
                    ...
                    with st.spinner("🧠 Analysing with AI..."):
                        results, all_preds, classes = predict(model, le, image)
                    top  = results[0]
                    ...
                    with col2:
                        ...
                        <div class='result-hero'>
                            ...
                            <p style='margin:1rem 0 0;color:#cbd5e0;font-size:0.92rem;line-height:1.6'>{info.get('description','')}</p>
                        </div>""", unsafe_allow_html=True)
                        ...
                        if top['referral']:
                            ...
                            <div class='referral-alert'>
                                ...
                                <span style='font-size:0.9rem'>This condition requires professional evaluation.
                                Please consult a certified dermatologist as soon as possible.</span>
                            </div>""", unsafe_allow_html=True)
                        ...
                        bcolors= [r['color'] for r in results]
                        bars   = ax.barh(names, confs, color=bcolors, alpha=0.9, edgecolor='none', height=0.5)
                        ...
                        st.markdown(f"<div class='treat-card'><h4>Recommended Actions</h4>{treat_html}</div>", unsafe_allow_html=True)

                        if info.get('risk_factors'):
                            st.markdown("<div class='section-title'>🔍 Risk Factors</div>", unsafe_allow_html=True)
                            rf = " ".join([f"<span style='background:rgba(245,101,101,0.15);color:#f56565;padding:3px 10px;border-radius:20px;font-size:0.85rem;margin:2px;display:inline-block'>{r}</span>"
                                           for r in info['risk_factors']])
                            st.markdown(f"<div style='margin:0.5rem 0'>{rf}</div>", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    st.markdown("<div class='main-hero'><h1>📅 Weekly Monitoring</h1><p>Track your skin condition over time — improving or worsening?</p></div>", unsafe_allow_html=True)
    if not st.session_state.history:
        ...
        <div style='background:linear-gradient(135deg,#1a1a2e,#16213e);border-radius:16px;
            ...
            upload an image, check <b style='color:#f56565'>Add to weekly monitoring tracker</b> and run analysis.</div>
        </div>""", unsafe_allow_html=True)
    else:
        df    = pd.DataFrame(st.session_state.history)
        spot  = st.selectbox("Select monitoring spot", df['spot'].unique())
        ...
        ax.set_xticks(x)
        ax.set_xticklabels([f"Visit {i+1}\n{sdf['date'].iloc[i][:10]}" for i in x],fontsize=8,color='#a0aec0')
        ax.set_ylabel('AI Confidence (%)',color='#a0aec0',fontsize=10)
        ax.set_ylim(0,110); ax.tick_params(colors='#a0aec0')
        for sp in ['top','right']: ax.spines[sp].set_visible(False)
        for sp in ['bottom','left']: ax.spines[sp].set_color('#4a5568')
        ...
        if len(sdf)>=2:
            trend=sdf['confidence'].iloc[-1]-sdf['confidence'].iloc[0]
            if trend>2:   st.markdown('<div class="sev-high">⚠️ <b>Condition WORSENING.</b> Please consult a dermatologist soon.</div>',unsafe_allow_html=True)
            elif trend<-2:st.markdown('<div class="sev-low">✅ <b>Condition IMPROVING.</b> Continue monitoring and treatment.</div>',unsafe_allow_html=True)
            else:         st.markdown('<div class="sev-medium">➡️ <b>Condition STABLE.</b> Continue weekly monitoring.</div>',unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    for i,(code,info) in enumerate(DISEASE_INFO.items()):
        with cols[i%2]:
            badge={'HIGH':'badge-high','MEDIUM':'badge-medium','LOW':'badge-low'}.get(info['severity'],'badge-low')
            treats="".join([f"<li style='color:#cbd5e0;margin:3px 0'>{t}</li>" for t in info['treatment'][:3]])
            st.markdown(f"""
            <div style='background:linear-gradient(135deg,#1a1a2e,#16213e);border-radius:12px;
                 padding:1.2rem;margin-bottom:1rem;border-left:4px solid {info["color"]};
                ...
                <div style='display:flex;align-items:center;gap:10px;margin-bottom:0.5rem'>
                    ...
                    <div>
                        ...
                        <span class='{badge}'>{info['severity']} RISK</span>
                    </div>
                </div>
                <p style='color:#a0aec0;font-size:0.85rem;margin:0.5rem 0;line-height:1.5'>{info['description']}</p>
                <ul style='margin:0.5rem 0;padding-left:1.2rem'>{treats}</ul>
            </div>""", unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        <p style='color:#cbd5e0;line-height:1.7'>SkinSense AI addresses the global dermatology crisis —
        <b>3 billion people</b> worldwide lack access to skin specialists. Using CNN-based deep learning
        trained on 10,000+ clinical images, this app provides instant skin disease detection with
        clinical-grade features including monitoring and doctor referral.</p>
        </div>
    ...
    with c2:
        ...
        <p style='color:#cbd5e0'>• <b style='color:white'>Framework:</b> TensorFlow / Keras → ONNX</p>
        <p style='color:#cbd5e0'>• <b style='color:white'>Runtime:</b> ONNX Runtime</p>
        ...
        <p style='color:#fed7d7;font-size:0.9rem;line-height:1.6'>This application is built for
        <b>educational purposes only</b>. It is NOT a substitute for professional medical advice.
        Always consult a qualified dermatologist for any skin concerns.</p>
        </div>""", unsafe_allow_html=True)
